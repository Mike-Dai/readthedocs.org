...
.. note::
   ...
   If you use conda, the python version doesn't have any effect, that's managed by conda.
...
.. note::
   ...
   If you use a conda environment, we don't install requirements from files here. You have to put the pip requirements inside the ``environment.yml`` used by conda.
...
.. warning::
   ...
   If you install your project with conda, you should not set the value to ``True``. Because the virtual environment creation in managed by conda. So, ``syste_packages`` does't have any effect.
